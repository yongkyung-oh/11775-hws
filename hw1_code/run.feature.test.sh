...
video_path=/home/ubuntu/videos   # path to the directory containing all the videos. In this example setup, we are linking all the videos to "../video"
cluster_num=180        # the number of clusters in k-means. Note that 50 is by no means the optimal solution.
...
#python scripts/select_frames.py list/train.video 0.5 select.mfcc.csv || exit 1;
