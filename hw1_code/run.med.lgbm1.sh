...
for event in P001 P002 P003; do
  ...
  python scripts/train_lgbm.py $event "kmeans/" $feat_dim_mfcc mfcc_pred/lgbm.$event.model False || exit 1;
...
for event in P001 P002 P003; do
  echo "=========  Event $event  ========="
  # now train a lgbm model
  python scripts/train_lgbm.py $event "asrfeat/" $feat_dim_asr asr_pred/lgbm.$event.model False || exit 1;
  # apply the lgbm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred 
...
for event in P001 P002 P003; do
  echo "=========  Event $event  ========="
  # now train a lgbm model
  python scripts/train_lgbm.py $event "all/" $feat_dim_asr all_pred/lgbm.$event.model False || exit 1;
  # apply the lgbm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred 
